...
class v8QBBLoss(v8DetectionLoss):
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
        """Preprocess targets for quadrant bounding box detection."""
        if targets.shape[0] == 0:
            out = torch.zeros(batch_size, 0, 6, device=self.device)
        else:
            i = targets[:, 0]  # image index
    ...
    def __call__(self, preds, batch):
        """Calculate and return the loss for quadrant bounding box detection."""
        loss = torch.zeros(3, device=self.device)  # box, cls, dfl
        feats, pred_angle = preds if isinstance(preds[0], list) else preds[1]
        batch_size = pred_angle.shape[0]  # batch size, number of masks, mask height, mask width
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
        ...
        try:
            ...
            raise TypeError(
                ...
                "i.e. 'yolo train model=yolo11n-qbb.pt data=dota8.yaml'.\nVerify your dataset is a "
