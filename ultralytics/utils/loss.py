...
class v8QBBLoss(v8DetectionLoss):
    ...
    def __init__(self, model):
        """Initialize v8QBBLoss with model, assigner, and rotated bbox loss; model must be de-paralleled."""
        super().__init__(model)
        self.assigner = RotatedTaskAlignedAssigner(topk=10, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = RotatedBboxLoss(self.reg_max).to(self.device)
    ...
    def __call__(self, preds, batch):
        ...
        try:
            batch_idx = batch["batch_idx"].view(-1, 1)
            # QBB: Handle 6-column data (class, cx, cy, w, h, angle)
            bbox_data = batch["bboxes"]
            if bbox_data.shape[-1] == 6:
                # 6-column format: class cx cy w h angle - take all 6 columns but skip class (first column handled separately)
                targets = torch.cat((batch_idx, batch["cls"].view(-1, 1), bbox_data.view(-1, 6)[:, 1:]), 1)
            else:
                # Fallback to 5-column format
                targets = torch.cat((batch_idx, batch["cls"].view(-1, 1), bbox_data.view(-1, 5)), 1)
            ...
            raise TypeError(
                ...
                "i.e. 'yolo train model=yolo11n-qbb.pt data=coco8.yaml'.\nVerify your dataset is a "
                "correctly formatted 'QBB' dataset using 6-column format (class cx cy w h angle) "
                "as an example.\nSee https://docs.ultralytics.com/datasets/obb/ for help."
            ) from e
