...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def __call__(self, preds: Any, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            raise TypeError("ERROR ❌ segment dataset incorrectly formatted or not a segment dataset.") from e
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def __call__(self, preds: Any, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            raise TypeError("ERROR ❌ OBB dataset incorrectly formatted or not a OBB dataset.") from e
...
class v8QBBLoss(v8OBBLoss):
    ...
    def preprocess(self, targets: torch.Tensor, batch_size: int, scale_tensor: torch.Tensor) -> torch.Tensor:
        if targets.shape[0] == 0:
            return torch.zeros(batch_size, 0, 9, device=self.device)
        i = targets[:, 0]
        _, counts = i.unique(return_counts=True)
        counts = counts.to(dtype=torch.int32)
        ...
        for j in range(batch_size):
            ...
            if n := matches.sum():
                ...
                qbboxes[..., 0::2] *= scale_tensor[1]
                qbboxes[..., 1::2] *= scale_tensor[0]
    ...
    def __call__(self, preds: Any, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            gt_labels, gt_bboxes = targets.split((1, 8), 2)
    ...
    def qbb_decode(self, anchor_points, pred_qbb):
        ...
        decoded_qbb[..., 0::2] += anchor_points[..., [0]]
        decoded_qbb[..., 1::2] += anchor_points[..., [1]]
        return decoded_qbb
