...
def on_predict_postprocess_end(predictor: object, persist: bool = False) -> None:
    ...
    for i, result in enumerate(predictor.results):
        ...
        det = (result.obb if predictor.args.task == "obb" else result.qbb if predictor.args.task == "qbb" else result.boxes).cpu().numpy()
        ...
        update_args = {("obb" if predictor.args.task == "obb" else "qbb" if predictor.args.task == "qbb" else "boxes"): torch.as_tensor(tracks[:, :-1])}
