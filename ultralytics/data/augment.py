...
class Format(BaseTransform):
    ...
    This class formats the labels for training, including bounding box normalization, mask handling, and color space
    conversion.

    Attributes:
        bbox_format (str): The format of the bounding box coordinates, either 'xyxy' or 'xywh'.
        normalize (bool): Whether to normalize the bounding box coordinates.
        return_mask (bool): Whether to return the segmentation masks.
        return_keypoint (bool): Whether to return the keypoints for pose estimation.
        return_obb (bool): Whether to return the oriented bounding boxes.
        return_qbb (bool): Whether to return the quadrilateral bounding boxes.
        batch_idx (bool): Whether to return the batch index.
        mask_ratio (int): The downsample ratio of the segmentation masks.
        mask_overlap (bool): Whether to overlap the segmentation masks.
        bgr (float): The probability of converting RGB to BGR.
    """
    ...
    def __init__(
        ...
        """
        Initialize the Format object for YOLOv8 segmentation model.
    ...
    def __call__(self, labels: Dict[str, Any]) -> Dict[str, Any]:
        """Apply formatting to labels, including bounding box normalization, mask handling, and color space conversion."""
        img = labels.pop("img")
        h, w = img.shape[:2]
        if self.bgr > 0 and random.random() < self.bgr:
            img = img[..., ::-1]
        # `convert_bbox` is not used here since the bbox format is already xywh.
        ...
        if instances:
            ...
            if self.return_obb:
                ...
                labels["bboxes"] = xywhr2xyxy(labels["obb"])  # TODO: check if this is correct
            
            if self.return_qbb:
                labels["qbb"] = instances.bboxes.copy()
                qbb = labels["qbb"]
                x_coords = qbb[:, 0::2]
                y_coords = qbb[:, 1::2]
                x1 = x_coords.min(1)
            ...
            # Return bboxes as xywh, normalized
            if "bboxes" in labels:
    ...
    def _format_img(self, img: np.ndarray) -> torch.Tensor:
        """Format image from HWC-BGR to CHW-RGB and converts to tensor."""
        if img.ndim == 2:
            img = img[..., None]
