...
# Ensure ultralytics package is in the path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from ultralytics import YOLO
from ultralytics.data.dataset import QuadrilateralDataset
...
if __name__ == "__main__":
    ...
    # 데이터셋 설정 파일 경로
    data = 'ultralytics/qyolo_data.yaml'
    ...
    model.train(
        ...
        batch=8,  # 배치 크기 조정
        workers=4,  # 워커 수 조정
        device=select_device('0'),  # GPU 설정
        save_period=10,  # 10 에폭마다 모델 저장
        patience=20,  # 얼리 스토핑 patience
    )
