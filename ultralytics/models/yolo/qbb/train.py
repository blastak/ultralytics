...
class QBBTrainer(yolo.detect.DetectionTrainer):
    """
    A class extending the DetectionTrainer class for training based on a Quadrilateral Bounding Box (QBB) model.

    This trainer specializes in training YOLO models that detect quadrilateral bounding boxes, which are useful for
    detecting objects with flexible four-point bounding boxes.
    ...
    Examples:
        ...
        >>> args = dict(model="yolov8n-qbb.pt", data="dota8.yaml", epochs=3)
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides: Optional[dict] = None, _callbacks: Optional[List[Any]] = None):
        """
        Initialize a QBBTrainer object for training Quadrilateral Bounding Box (QBB) models.
    ...
    def get_validator(self):
        ...
        self.loss_names = "qbox_loss", "cls_loss", "coord_loss"
