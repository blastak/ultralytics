...
def create_8point_obb_dataset():
    """8-point OBB 형식 데이터셋 생성"""
    
    source_dir = Path('/workspace/DB/01_LicensePlate/55_WebPlatemania_1944/all')
    target_dir = Path('webpm_obb8_dataset')
...
"""
    ...
    yaml_file = target_dir / 'webpm_obb8.yaml'
...
def main():
    print("Creating 8-point OBB dataset...")
    dataset_dir, yaml_file = create_8point_obb_dataset()
    ...
    print(f"\nStarting OBB training with user's method...")
    
    # 사용자 방식으로 학습
    try:
        model = YOLO('yolo11n-obb.pt')  # 사용자는 .yaml을 썼지만 .pt가 더 안전
        results = model.train(
            data=str(yaml_file), 
            epochs=10, 
            imgsz=640, 
        ...
        print("✅ OBB training completed successfully!")
        ...
        if Path(best_model_path).exists():
            ...
            test_obb_model(best_model_path, dataset_dir)
...
def test_obb_model(model_path, dataset_dir):
    """OBB 모델 테스트"""
    
    print(f"\nTesting OBB model: {model_path}")
    ...
    try:
        ...
        for i, img_path in enumerate(test_images):
            ...
            # 저장
            save_path = vis_dir / f'obb_test_{i+1:02d}_{img_path.name}'
