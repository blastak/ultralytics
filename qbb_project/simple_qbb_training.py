...
def create_simple_qbb_dataset():
                    ...
                    for shape in labelme_data['shapes']:
                        if shape['shape_type'] == 'polygon' and len(shape['points']) >= 3:
                            ...
                            try:
                                ...
                                # YOLOv11 QBB 형식: class cx cy w h rotation
    ...
    # YAML 설정 파일 생성
    yaml_content = f"""# Simple YOLOv11 QBB dataset
...
"""
    ...
    yaml_file = target_dir / 'simple_qbb.yaml'
...
def train_simple_qbb():
    """간단한 QBB 모델 학습"""
    
    print("Creating simple QBB dataset...")
    dataset_dir, yaml_file = create_simple_qbb_dataset()
    
    print("Starting simple QBB training...")
    print(f"CUDA available: {torch.cuda.is_available()}")
    
    # YOLOv11n-qbb 모델 로드 (task 자동 인식)
    ...
    try:
        ...
        print("Simple QBB training completed successfully!")
        print(f"Results saved in: runs/qbb/simple_train/")
        
        # 모델 테스트
        best_model_path = 'runs/qbb/simple_train/weights/best.pt'
...
def test_trained_model(model_path, dataset_dir):
    ...
    print(f"\nTesting trained QBB model: {model_path}")
    ...
    try:
        ...
        vis_dir = Path('simple_qbb_test_results')
        ...
        for i, img_path in enumerate(test_images):
            ...
            for r in results:
                if r.qbb is not None and len(r.qbb) > 0:
                    ...
                    print(f"  Detected {len(r.qbb)} license plates")
                else:
...
def main():
    print("Starting simple QBB training approach...")
    
    success = train_simple_qbb()
    
    if success:
        print("\n✅ Simple QBB training completed successfully!")
        print("Next steps:")
        print("1. Check results in runs/qbb/simple_train/")
        print("2. Review test results in simple_qbb_test_results/")
        print("3. Scale up with more data if needed")
    else:
        print("\n❌ Simple QBB training failed")
