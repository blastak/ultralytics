#!/usr/bin/env python3
"""
번호판 QBB 모델로 false alarm과 miss detection 분석
"""
...
def analyze_qbb_performance():
    ...
    # 결과 저장 디렉토리
    vis_dir = Path('qbb_analysis_results')
    if vis_dir.exists():
    ...
    for i, json_file in enumerate(json_files):
        ...
        try:
            ...
            if len(correct_detections) <= 10 and has_gt and has_detection:
                visualize_qbb_result(img_file, json_file, results, correct_detection_dir, "correct")
            elif len(miss_detections) <= 10 and has_gt and not has_detection:
                visualize_qbb_result(img_file, json_file, results, miss_detection_dir, "miss")
            elif len(false_alarms) <= 10 and not has_gt and has_detection:
                visualize_qbb_result(img_file, json_file, results, false_alarm_dir, "false_alarm")
...
def visualize_qbb_result(img_file, json_file, results, save_dir, result_type):
    """QBB 결과 시각화"""
        ...
        for r in results:
            if r.qbb is not None and len(r.qbb) > 0:
                for j in range(len(r.qbb)):
                    ...
                    cv2.putText(img, f'QBB:{conf:.2f}', (center[0]-30, center[1]), 
...
def test_navertaxi_qbb():
    ...
    for conf_thresh in confidence_thresholds:
        ...
        # 저장
        save_path = f'navertaxi_qbb_conf_{conf_thresh}.jpg'
