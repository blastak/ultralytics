#!/usr/bin/env python3
"""
Normal IoU (AABB) 기반 mAP 평가
표준 detection 논문에서 사용하는 AABB IoU 기반 평가
AABB, OBB, QBB 모두 AABB로 변환하여 평가
"""
    ...
    for label_file in label_path.glob("*.txt"):
        ...
        for line in lines:
            ...
            if len(parts) >= 9:  # class + 8 coords
                ...
                boxes.append({
                    'class': class_id,
                    'coords': coords,  # normalized xyxyxyxy
                    'matched': False
                })
            ...
            for row in reader:
                if len(row) >= 10:  # class + 8 coords + confidence
                    ...
                    detections.append({
                        ...
                        'coords': coords,  # absolute coords (xyxyxyxy)
...
def xyxyxyxy_to_xyxy(coords):
    """
    xyxyxyxy (8좌표) -> xyxy (AABB) 변환
...
def evaluate_map(gt_dict, pred_dict, iou_threshold=0.5, img_width=720, img_height=1160):
    ...
    for idx, pred in enumerate(tqdm(all_predictions, desc="Calculating IoU")):
        ...
        # 예측 박스를 AABB로 변환
        pred_aabb = xyxyxyxy_to_xyxy(pred['coords'])
        ...
        for gt_idx, gt_box in enumerate(gt_boxes):
            ...
            # GT를 AABB로 변환
            gt_aabb = xyxyxyxy_to_xyxy(gt_coords)

            iou = calculate_iou(pred_aabb, gt_aabb)
...
def main():
    ...
    메인 평가 함수
    """
    parser = argparse.ArgumentParser(description='Normal IoU (AABB) 기반 mAP 평가')
